...
products.open('GET','../data/products.json')
products.onload = function(){
  console.log(a.responseText);
};
products.send()   L'erreur Access to XMLHttpRequest at 'file:///C:/data/products.json' from 
origin 'null' has been blocked by CORS policy: Cross origin requests are only supported for protocol s
chemes: http, data, isolated-app, chrome-extension, chrome, https, chrome-untrusted.
 était constament appelé donc nous avons fini par mettre les données du fichier json dans le local storage**/
 const products = [
  {
    "id": 1,
    "name": "Apple TV",
...
$(document).ready(() => {
  ...
  products.forEach(product => {
    let productElement = $('<div>').addClass('product').html(`
    ...
    `);
    $('#products-list').append(productElement);
  });

  $('#product-criteria button').click(function() {
    let criteria = $(this).text().toLowerCase();
  ...
  $('#photo').click(function(){
    ...
    let categorie = products.filter(product => product.category === "cameras");
    renderProducts(categorie);
    $('#photo').addClass('selected');
    $('#all').removeClass('selected');
    $('#console').removeClass('selected');
    $('#écran').removeClass('selected');
    $('#ordi').removeClass('selected');
  })
  $('#console').click(function(){
    let categorie = products.filter(product => product.category === "consoles");
    renderProducts(categorie);
  ...
  $('#écran').click(function(){ 
    let categorie = products.filter(product => product.category === "screens");
    renderProducts(categorie);
    $('#écran').addClass('selected');
    $('#all').removeClass('selected');
    $('#console').removeClass('selected');
    $('#photo').removeClass('selected');
    $('#ordi').removeClass('selected');
  ...
  $('#ordi').click(function(){ 
    let categorie = products.filter(product => product.category === "computers");
    renderProducts(categorie);
  ...
  function renderProducts(produitMontre) {
    $('#products-list').empty();
    for (let i = 0; i < produitMontre.length; i++) {
      let product = produitMontre[i];
      let productElement = $('<div>').addClass('product').html(`
        <a href="./product.html" id = "${product.id}">
          <h2>${product.name}</h2>
          <img alt="${product.name}" src="./assets/img/${product.image}">
      ...
      `);
      $('#products-list').append(productElement);
      let numProducts = $('#products-list .product').length;
      $('#products-count').text(numProducts + ' produits');
      $('#products-count').text(numProducts + ' produits');
    }
  }});
